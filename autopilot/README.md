...
cron ─▶ autopilot.py report
          인스타 인사이트 + 쿠팡 커미션 리포트 → shorts/learnings.md → 다음 source/script 프롬프트에 주입
```
...
## 링크 페이지 (프로필 링크)
인스타 캡션의 URL은 클릭되지 않으므로, 영상 끝에 "프로필 링크 N번"을 안내하고 `docs/index.html` 에서 번호로 찾게 합니다.
GitHub 저장소 Settings → Pages → Branch 의 `/docs` 폴더로 배포하고, 그 주소를 인스타 프로필 링크에 넣으세요.
